...
def decoder_output_to_image(decoder_output, pipe):
    image = (decoder_output / 2 + 0.5).clamp(0, 1)
    image = image.cpu().permute(0, 2, 3, 1).numpy()
    pipe.numpy_to_pil(image)
...
def waveform_from_spectrogram(mel_amplitudes):
    mel_amplitudes_torch = torch.from_numpy(mel_amplitudes)
    amplitudes_linear = inverse_mel_scaler(mel_amplitudes_torch)
    return inverse_spectrogram_func(amplitudes_linear)
